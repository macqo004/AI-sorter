...
class ImageExtensionFixer:
    ...
    rejects the complete plan before filesystem changes begin.
    """
        ...
        try:
            for index, proposal in enumerate(proposals):
                ...
                temporary_path = proposal.source.with_name(
                    f"{self.TEMP_PREFIX}{index:06d}{self.TEMP_SUFFIX}"
                )
        ...
        try:
            with self.database.transaction() as connection:
                for _temporary_path, destination, proposal in temporary:
                    ...
                    if row is None:
                        raise DatabaseError(
                            f"Nie znaleziono źródłowej lokalizacji w bazie danych: {source_text}"
                        )
                    db_sha = str(row["sha512"]).lower()
                    if db_sha != proposal.sha512:
                        raise DatabaseError(
                            f"SHA-512 źródła zmieniło się przed aktualizacją bazy: {source_text}"
                        )
                    ...
                    if destination_row is not None:
                        raise DatabaseError(
                            f"Docelowa lokalizacja pojawiła się w bazie danych: {destination_text}"
                        )
                    cursor = connection.execute(
                        """
                        UPDATE file_location
                        SET absolute_path = ?, location_status = 'ACTIVE'
                    ...
                    if cursor.rowcount != 1:
                        raise DatabaseError(
                            f"Nie udało się zaktualizować lokalizacji po zmianie rozszerzenia: {destination_text}"
                        )
        except DatabaseError:
            raise
        except Exception as exc:
            raise DatabaseError(
                "Nie udało się zaktualizować bazy danych po zmianie rozszerzenia."
            ) from exc
