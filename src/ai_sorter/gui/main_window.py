...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self._color_scope_root: Path | None = None
    ...
    def _choose_color_scope(self) -> Path | None | False:
        dialog = QMessageBox(self)
        dialog.setWindowTitle("Color / BW Analysis — scope")
        dialog.setText("Wybierz zakres analizy.")
        whole_button = dialog.addButton("Cały projekt", QMessageBox.AcceptRole)
        folder_button = dialog.addButton("Wybrany folder…", QMessageBox.ActionRole)
        ...
        if clicked is folder_button:
            root = QFileDialog.getExistingDirectory(self, "Choose Color Analysis folder")
            return Path(root).resolve() if root else False
        return False
    ...
    def start_color_analysis(self) -> None:
        scope = self._choose_color_scope()
        if scope is False:
            return

        self._color_scope_root = scope
        self.close_after_color = False
        ...
        self.progress.setRange(0, 0)
        scope_text = str(scope) if scope else "Entire project"
        ...
        self.color_worker = ColorAnalysisWorker(analyzer, scope)
    ...
    def _render_color_progress(self, progress: ColorProgress) -> None:
        ...
        scope_text = str(self._color_scope_root) if self._color_scope_root else "Entire project"
        self.scan_details.setText(
            "Color Analysis\n"
            f"Scope: {scope_text}\n"
            f"Processed: {progress.processed}\n"
    ...
    def _format_color_summary(self, summary: ColorSummary, title: str) -> str:
        ...
        scope_text = summary.scope_root or "Entire project"
        return (
            f"{title}\n\n"
            f"Scope: {scope_text}\n"
