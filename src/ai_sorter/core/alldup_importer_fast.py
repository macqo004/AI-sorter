"""Fast AllDup SHA-512 cache importer using batched indexed lookups."""
...
class AllDupImporterFast:
    ...
    def run(
        ...
        try:
            query = (
                ...
                "FROM file_location WHERE location_status = 'ACTIVE' "
            ...
            for offset in range(0, len(locations), BATCH_SIZE):
                ...
                paths = [str(row["absolute_path"]) for row in batch]
                if not paths:
                    continue
                ...
                for row in rows:
                    ...
                    if project_row is None:
                        continue
                    project_size = int(project_row["file_size"] or -1)
                    alldup_size = int(row["fsize"] or -2)
                    if project_size != alldup_size:
                        continue
                    digest = self._normalize_checksum(row["checksum"])
                    if len(digest) == 128:
                        exact_matches[path] = digest

            exact = len(exact_matches)
            suffix_matches = ambiguous_matches = no_path_match = 0
            sha512_available = unchanged = conflicts = cacheable = 0
            cache_rows: list[tuple[str, int, str]] = []

            unmatched = [row for row in locations if str(row["absolute_path"]) not in exact_matches]

            for path, alldup_sha in exact_matches.items():
                row = by_path[path]
            ...
            return ImportStats(
                ...
                no_path_match=no_path_match,
                sha512_available=sha512_available,
                unchanged=unchanged,
                conflicts=conflicts,
                cacheable=cacheable,
                imported=imported,
    ...
    def _ensure_cache_table(connection: sqlite3.Connection) -> None:
        ...
        connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_external_hash_cache_path "
            "ON external_hash_cache(absolute_path)"
        )
    ...
    def _find_suffix_candidates(
        cls, connection: sqlite3.Connection, path: str, size: int
