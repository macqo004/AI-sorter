...
class Database:
    ...
    def upsert_file(self, record: FileRecord) -> None:
        ...
        try:
            ...
            connection.rollback()
            raise DatabaseError(
                "Nie udało się zapisać identyfikatora pliku w bazie danych."
            ) from exc
    ...
    def upsert_file_location(self, record: FileLocationRecord) -> None:
        ...
        try:
            ...
            connection.rollback()
            raise DatabaseError(
                "Nie można zapisać lokalizacji, ponieważ jej plik nie ma zarejestrowanej tożsamości."
            ) from exc
        except sqlite3.Error as exc:
            connection.rollback()
            raise DatabaseError(
                "Nie udało się zapisać lokalizacji pliku w bazie danych."
            ) from exc
    ...
    def register_module(self, record: ModuleRecord) -> None:
        ...
        try:
            ...
            connection.rollback()
            raise DatabaseError(
                "Nie udało się zarejestrować modułu w bazie danych."
            ) from exc
    ...
    def start_module_execution(self, module_id: str, started_at: datetime) -> int:
        ...
        try:
            cursor = connection.execute(
                """
                INSERT INTO module_execution
                    (module_id, started_at, status)
                VALUES (?, ?, 'RUNNING')
                """,
            ...
            connection.rollback()
            raise DatabaseError(
                "Nie można uruchomić wykonania modułu, ponieważ moduł nie jest zarejestrowany."
            ) from exc
        except sqlite3.Error as exc:
            connection.rollback()
            raise DatabaseError(
                "Nie udało się zapisać rozpoczęcia wykonania modułu."
            ) from exc

    def finish_module_execution(
        self,
        ...
        try:
            ...
            connection.rollback()
            raise DatabaseError(
                "Nie udało się zapisać zakończenia wykonania modułu."
            ) from exc
    ...
    def record_module_result(
        ...
        try:
            ...
            connection.rollback()
            raise DatabaseError(
                "Nie udało się zapisać wyniku modułu w bazie danych."
            ) from exc
    ...
    def _migrate(self) -> None:
        ...
        if current > SCHEMA_VERSION:
            raise DatabaseError(
                f"Baza projektu pochodzi z nowszej wersji aplikacji (schema {current}). "
                "Ta wersja programu nie może jej bezpiecznie otworzyć."
            )
        try:
            if current < 2:
                connection.executescript(
                    ...
                    CREATE TABLE IF NOT EXISTS file_location (
                        ...
                        FOREIGN KEY (sha512) REFERENCES file_record(sha512)
                            ON DELETE CASCADE
                    );
                    ...
                    CREATE INDEX IF NOT EXISTS idx_file_location_path
                        ON file_location(absolute_path);
                    CREATE INDEX IF NOT EXISTS idx_module_execution_module
                        ON module_execution(module_id);
                    CREATE INDEX IF NOT EXISTS idx_analysis_result_module
                        ON analysis_result(module_id);
            ...
            connection.rollback()
            raise DatabaseError(
                "Nie udało się zaktualizować schematu bazy danych projektu. "
                "Baza nie została pozostawiona w częściowo zmigrowanym stanie."
            ) from exc
    ...
    def _require_connection(self) -> sqlite3.Connection:
        if self.connection is None:
            raise DatabaseError(
                "Baza danych projektu nie jest obecnie połączona."
            )
    ...
    def _validate_sha512(value: str) -> None:
        ...
        if len(normalized) != 128 or any(character not in "0123456789abcdef" for character in normalized):
            raise DatabaseError(
                "Podany identyfikator SHA512 ma nieprawidłowy format."
            )
