...
from .models import DatabaseStatus, FileLocationRecord, FileRecord, ModuleExecutionRecord, ModuleRecord
        ...
        try:
            connection.execute(
                ...
                (record.sha512.lower(), record.absolute_path, record.file_size,
                 self._iso(record.modified_at), record.location_status, record.last_seen_execution_id),
            )
            connection.commit()
    ...
    def record_module_result(self, sha512: str, module_id: str, result_key: str, payload: dict, confidence: float | None = None) -> None:
        ...
        try:
            connection.execute(
                ...
                """,
                (sha512.lower(), module_id, result_key, confidence, json.dumps(payload, ensure_ascii=False, sort_keys=True)),
            )
            connection.commit()
        except sqlite3.Error as exc:
            connection.rollback()
