...
class SimImagesFilesystemAnalysis:
    ...
    def format_text(self) -> str:
        lines = [
            ...
            f"Requested rows: {self.requested_rows:,}",
...
class SimImagesFilesystemProbe:
    ...
    def analyze(
        ...
        for row in rows:
            if inspected >= max_rows_to_probe:
                break
            inspected += 1
            ...
            if resolved is None:
                ...
                try:
                    ...
                    data = row["data"]
                    if (
                        blob_sample_size > len(blob_samples)
                        and isinstance(data, (bytes, bytearray, memoryview))
                    ):
                        blob = bytes(data)
                        blob_samples.append(
                            (int(row["mid"]), file_name, len(blob), blob[:84].hex())
                        )
    ...
    def _logical_drive_roots() -> list[str]:
        ...
        written = ctypes.windll.kernel32.GetLogicalDriveStringsW(length + 1, buffer)
        if written <= 0:
            return roots
        for root in buffer[:written].split("\x00"):
            if root:
                roots.append(root)
        return roots
    ...
    def _combine_path(root: str, folder: str, file_name: str) -> str:
        ...
        return str(root_path / relative_folder / relative_file)
